...
reviews = df.iloc[0:20,0] # iloc[행,열]
title = df.iloc[0:20,1]

# 둘다 (20,)
no =[i for i in range(1,21)]
...
def review() : # 호출 함수
    title_review = zip(no, title, reviews)
    return render_template('/exam02/result.html', title_review=title_review) # result 페이지 요청   
